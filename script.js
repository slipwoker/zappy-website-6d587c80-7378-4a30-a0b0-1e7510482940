document.addEventListener('DOMContentLoaded', function () {
  ...
  const validationMessages = {
    ...
    minLength: isRTL ? 'يجب أن يحتوي هذا الحقل على {min} أحرف على الأقل' : 'This field must be at least {min} characters',
  ...
  function validateField(field) {
    ...
    if (value !== '') {
      ...
      if (minLength && value.length < parseInt(minLength, 10)) {
        const msg = validationMessages.minLength.replace('{min}', minLength);
  ...
  function showFormSuccess(form) {
    ...
    successEl.textContent = validationMessages.success;
  ...
  document.addEventListener('blur', function (e) {
    ...
    if (!field.closest('.contact-form')) return;
    if (field.type === 'submit' || field.type === 'button' || field.type === 'reset' || field.disabled) return;
  ...
  const OBSERVED_CLASS = 'scroll-animate';

  function initScrollAnimations() {
    const elements = document.querySelectorAll(
      '.fade-in, .slide-up, .slide-in, [data-animate], .scroll-animate, ' +
      '.feature-card, .service-card, .pricing-card, .testimonial-card, ' +
      '.team-card, .portfolio-item, .blog-card, .stat-item, .step-item'
    );
    ...
    const observer = new IntersectionObserver(function (entries) {
      entries.forEach(function (entry) {
        if (entry.isIntersecting) {
          ...
          const delay = el.getAttribute('data-delay') || el.getAttribute('data-animate-delay') || 0;
    ...
    elements.forEach(function (el, index) {
      el.classList.add(OBSERVED_CLASS);

      if (!el.getAttribute('data-delay') && !el.getAttribute('data-animate-delay')) {
        const siblings = Array.from(el.parentNode.children).filter(function (child) {
          return child.classList.contains(OBSERVED_CLASS);
        });
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "T.D GLOBAL משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
    ...
    (function() {
        ...
        function initContactForm() {
            ...
            try {
                ...
                const payload = {
                    websiteId: '6d587c80-7378-4a30-a0b0-1e7510482940',
                    name: name,
                    email: email,
                    subject: subject,
                    message: message,
                ...
                if (result.success) {
                    ...
                    if (result.thankYouPagePath && result.ticketNumber) {
                        ...
                        if (isPreviewMode && window.ZAPPY_CONFIG) {
                            ...
                            const websiteId = window.ZAPPY_CONFIG.websiteId || '6d587c80-7378-4a30-a0b0-1e7510482940';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '6d587c80-7378-4a30-a0b0-1e7510482940';
